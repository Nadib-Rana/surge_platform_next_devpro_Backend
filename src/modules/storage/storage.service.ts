...
export class StorageService {
  ...
  async generatePresignedUploadUrl(dto: { contentType: string; folder: string; fileName: string }) {
    const objectName = `${dto.folder.replace(/^\/+|\/+$/g, '')}/${Date.now()}-${randomUUID()}-${dto.fileName}`;

    await this.ensureBucketExists();

    const uploadUrl = await this.minio.presignedPutObject(this.bucketName, objectName, 24 * 60 * 60);
  ...
  async getPresignedDownloadUrl(objectName: string) {
    await this.ensureBucketExists();
    return this.minio.presignedGetObject(this.bucketName, objectName, 24 * 60 * 60);
  }
