...
export class RssSchedulerService implements OnModuleInit {
  ...
  constructor(private config: ConfigService, private prisma: PrismaService) {
    ...
    this.queue = new Queue("rss-fetch-queue", { connection: this.redis as any });
  }

  async onModuleInit() {
    // On boot, register repeatable jobs for existing active feeds using workspace config
    const feeds = await this.prisma.rssFeed.findMany({ where: { status: "active" } });
  ...
  async ensureFeedScheduled(workspaceId: string, feedId: string, feedUrl: string) {
  ...
  private async scheduleRepeatableJob(workspaceId: string, feedId: string, feedUrl: string, freqHours: number) {
    // remove existing
  ...
  async onWorkspaceConfigChange(workspaceId: string) {
    ...
    const feeds = await this.prisma.rssFeed.findMany({ where: { workspaceId, status: "active" } });
    for (const f of feeds) {
