...
export class WordpressStrategy extends BaseDispatcher {
  ...
  async uploadMedia(siteUrl: string, imageUrl: string, credentials: Record<string, any>) {
    // For simplicity: if imageUrl is remote, fetch it and forward bytes to WP media endpoint
    try {
      const imageResp = await axios.get(imageUrl, { responseType: 'arraybuffer' });
      const form = new FormData();
      const filename = (imageUrl.split('/').pop() || 'upload.jpg').split('?')[0];
      form.append('file', Buffer.from(imageResp.data), { filename });

      const headers = { ...form.getHeaders(), ...this.getAuthHeader(credentials) };
      const mediaResp = await axios.post(`${siteUrl.replace(/\/$/, '')}/wp-json/wp/v2/media`, form, { headers });
  ...
  async dispatch(payload: DispatchPayload): Promise<DispatchResult> {
    ...
    if (!siteUrl) return { success: false, error: 'Missing WordPress site URL in credentials.apiUrl or siteUrl' };

    try {
      let featuredMediaId: number | undefined;
      if (payload.images && payload.images.length > 0) {
        const media = await this.uploadMedia(siteUrl, payload.images[0], payload.credentials);
      ...
      const resp = await axios.post(`${siteUrl.replace(/\/$/, '')}/wp-json/wp/v2/posts`, postBody, { headers });

      return { success: true, id: String(resp.data.id), url: resp.data.link, raw: resp.data };
    } catch (err: any) {
      return { success: false, error: err?.message || 'wordpress: unknown error', raw: err?.response?.data ?? err?.toString() };
    }
  }
}
