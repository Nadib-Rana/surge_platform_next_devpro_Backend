...
export class FacebookStrategy extends BaseDispatcher {
  ...
  async dispatch(payload: DispatchPayload): Promise<DispatchResult> {
    ...
    if (!token || !targetNode)
      return {
        success: false,
        error: "Missing access token or target node id",
      };
    ...
    try {
      ...
      if (payload.images && payload.images.length) {
        ...
        const resp = await axios.post(
          `https://graph.facebook.com/${targetNode}/photos`,
          params,
        );
        return {
          success: true,
          id: resp.data?.id,
          raw: resp.data,
          url: `https://facebook.com/${resp.data?.post_id || resp.data?.id}`,
        };
      }
      ...
      const feedResp = await axios.post(
        `https://graph.facebook.com/${targetNode}/feed`,
        feedParams,
      );
      return {
        success: true,
        id: feedResp.data?.id,
        raw: feedResp.data,
        url: `https://facebook.com/${feedResp.data?.id}`,
      };
    } catch (err: any) {
      return {
        success: false,
        error: err?.message || "facebook error",
