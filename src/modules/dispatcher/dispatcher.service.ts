...
export class DispatcherService {
  ...
  constructor() {
    ...
    this.strategies = [new WordpressStrategy(), new LinkedinStrategy(), new FacebookStrategy()];
  }
  ...
  async dispatch(payload: DispatchPayload): Promise<DispatchResult> {
    ...
    if (!strategy) return { success: false, error: `No dispatcher configured for channel: ${channel}` };
